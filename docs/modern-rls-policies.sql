...
DROP POLICY IF EXISTS "calendar_memos_select_policy" ON calendar_memos;
DROP POLICY IF EXISTS "calendar_memos_insert_policy" ON calendar_memos;
...
-- ========================================
-- 7. 메모 테이블 RLS 정책 (새로운 방식)
-- ========================================

-- 메모 조회: 작성자만 가능
CREATE POLICY "calendar_memos_select_policy" ON calendar_memos
...
-- 메모 생성: 인증된 사용자만 가능
CREATE POLICY "calendar_memos_insert_policy" ON calendar_memos
  FOR INSERT TO authenticated
  WITH CHECK (
    auth.jwt()->>'sub' IS NOT NULL AND
    user_id = auth.jwt()->>'sub'
...
-- 메모 수정: 작성자만 가능
CREATE POLICY "calendar_memos_update_policy" ON calendar_memos
  FOR UPDATE TO authenticated
  USING (
    user_id = auth.jwt()->>'sub'
  )
...
-- 메모 삭제: 작성자만 가능
CREATE POLICY "calendar_memos_delete_policy" ON calendar_memos
  FOR DELETE TO authenticated
  USING (
    user_id = auth.jwt()->>'sub'
  );
...
BEGIN
  ...
  WHERE t.table_schema = 'public' 
    AND t.table_name IN ('categories', 'posts', 'comments', 'likes', 'calendar_memos')
...
BEGIN
  ...
  RAISE NOTICE '  • calendar_memos: 2개 정책 (인증된 사용자 메모 조회 및 생성)';
  RAISE NOTICE '';
